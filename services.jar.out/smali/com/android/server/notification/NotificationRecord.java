...
public final class NotificationRecord {
    ...
    void dump(PrintWriter pw, String prefix, Context baseContext) {
        ...
        if (notification.actions != null && notification.actions.length > 0) {
            ...
            for (Action action : notification.actions) {
                pw.println(String.format("%s    [%d] \"%s\" -> %s", new Object[]{prefix, Integer.valueOf(i), action.title, action.actionIntent.toString()}));
            }
            pw.println(prefix + "  }");
        }
                ...
                if (val == null) {
                    ...
                    if (!((val instanceof CharSequence) || (val instanceof String))) {
                        if (val instanceof Bitmap) {
                            ...
                            pw.println(" (" + Array.getLength(val) + ")");
                        } else {
    ...
    public final String toString() {
        return String.format("NotificationRecord(0x%08x: pkg=%s user=%s id=%d tag=%s score=%d key=%s: %s)", new Object[]{Integer.valueOf(System.identityHashCode(this)), this.sbn.getPackageName(), this.sbn.getUser(), Integer.valueOf(this.sbn.getId()), this.sbn.getTag(), Integer.valueOf(this.sbn.getScore()), this.sbn.getKey(), this.sbn.getNotification()});
    }
