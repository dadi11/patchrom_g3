...
public class DowntimeCalendar {
    ...
    private final ArraySet<Integer> mDays;
    private DowntimeInfo mInfo;

    public DowntimeCalendar() {
    ...
    private boolean isInDowntime(int daysOffset, long time, long start, long end) {
        return this.mDays.contains(Integer.valueOf(((((getDayOfWeek(time) + -1) + (daysOffset % 7)) + 7) % 7) + 1)) && time >= addDays(start, daysOffset) && time < addDays(end, daysOffset);
    }
