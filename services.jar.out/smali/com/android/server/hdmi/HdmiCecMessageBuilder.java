...
public class HdmiCecMessageBuilder {
    ...
    static HdmiCecMessage buildSetOsdNameCommand(int src, int dest, String name) {
        try {
            return buildCommand(src, dest, 71, name.substring(0, Math.min(name.length(), OSD_NAME_MAX_LENGTH)).getBytes("US-ASCII"));
    ...
    static HdmiCecMessage buildTextViewOn(int src, int dest) {
        return buildCommand(src, dest, OSD_NAME_MAX_LENGTH);
    }
    ...
    static HdmiCecMessage buildRoutingChange(int src, int oldPath, int newPath) {
        return buildCommand(src, 15, DumpState.DUMP_PROVIDERS, new byte[]{(byte) ((oldPath >> 8) & 255), (byte) (oldPath & 255), (byte) ((newPath >> 8) & 255), (byte) (newPath & 255)});
    }
    ...
    static HdmiCecMessage buildReportPowerStatus(int src, int dest, int powerStatus) {
        return buildCommand(src, dest, HdmiCecKeycode.UI_SOUND_PRESENTATION_SELECT_AUDIO_AUTO_REVERBERATION, new byte[]{(byte) (powerStatus & 255)});
    }
    ...
    static HdmiCecMessage buildVendorCommandWithId(int src, int dest, int vendorId, byte[] operands) {
        ...
        return buildCommand(src, dest, HdmiCecKeycode.UI_SOUND_PRESENTATION_SELECT_AUDIO_AUTO_EQUALIZER, params);
    }
