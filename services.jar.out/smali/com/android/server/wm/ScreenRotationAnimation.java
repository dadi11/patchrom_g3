...
class ScreenRotationAnimation {
    ...
    /* renamed from: com.android.server.wm.ScreenRotationAnimation.H */
    final class C0560H extends Handler {
        public static final int FREEZE_TIMEOUT_VAL = 6000;
        public static final int SCREENSHOT_FREEZE_TIMEOUT = 2;

        public C0560H(Looper looper) {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case SCREENSHOT_FREEZE_TIMEOUT /*2*/:
                    synchronized (ScreenRotationAnimation.this.mLock) {
                        if (ScreenRotationAnimation.this.mSurfaceControl == null || !ScreenRotationAnimation.this.isAnimating()) {
                            if (ScreenRotationAnimation.this.mSurfaceControl != null) {
    ...
    public ScreenRotationAnimation(Context context, DisplayContent displayContent, SurfaceSession session, boolean inTransaction, boolean forceDefaultOrientation, boolean isSecure) {
        ...
        this.mHandler = new C0560H(DisplayThread.get().getLooper());
        ...
        int flags = 4;
        if (isSecure) {
            flags = 4 | DumpState.DUMP_PROVIDERS;
        }
        try {
    ...
    boolean hasScreenshot() {
        return this.mSurfaceControl != null ? true : DEBUG_TRANSFORMS;
    }
    ...
    private void setRotationInTransaction(int rotation) {
        ...
        createRotationMatrix(deltaRotation(rotation, 0), this.mWidth, this.mHeight, this.mSnapshotInitialMatrix);
    ...
    private boolean startAnimation(SurfaceSession session, long maxAnimationDuration, float animationScale, int finalWidth, int finalHeight, boolean dismissing, int exitAnim, int enterAnim) {
        ...
        if (exitAnim == 0 || enterAnim == 0) {
            boolean customAnim = DEBUG_TRANSFORMS;
            switch (delta) {
                case AppTransition.TRANSIT_NONE /*0*/:
                    this.mRotateExitAnimation = AnimationUtils.loadAnimation(this.mContext, 17432647);
                    this.mRotateEnterAnimation = AnimationUtils.loadAnimation(this.mContext, 17432646);
                    break;
                case MyHandler.MESSAGE_COMPUTE_CHANGED_WINDOWS /*1*/:
                    this.mRotateExitAnimation = AnimationUtils.loadAnimation(this.mContext, 17432659);
                    this.mRotateEnterAnimation = AnimationUtils.loadAnimation(this.mContext, 17432658);
                    break;
                case C0569H.REPORT_FOCUS_CHANGE /*2*/:
                    this.mRotateExitAnimation = AnimationUtils.loadAnimation(this.mContext, 17432650);
                    this.mRotateEnterAnimation = AnimationUtils.loadAnimation(this.mContext, 17432649);
                    break;
                case C0569H.REPORT_LOSING_FOCUS /*3*/:
                    this.mRotateExitAnimation = AnimationUtils.loadAnimation(this.mContext, 17432656);
                    this.mRotateEnterAnimation = AnimationUtils.loadAnimation(this.mContext, 17432655);
                    break;
                default:
                    break;
            }
        ...
        this.mAnimRunning = DEBUG_TRANSFORMS;
        this.mFinishAnimReady = DEBUG_TRANSFORMS;
        ...
        if (customAnim && this.mEnteringBlackFrame == null) {
            SurfaceControl.openTransaction();
            try {
                this.mEnteringBlackFrame = new BlackFrame(session, new Rect((-finalWidth) * 1, (-finalHeight) * 1, finalWidth * 2, finalHeight * 2), new Rect(0, 0, finalWidth, finalHeight), FREEZE_LAYER, layerStack, DEBUG_TRANSFORMS);
    ...
    public boolean isAnimating() {
        return !hasAnimations() ? DEBUG_TRANSFORMS : true;
    }

    public boolean isRotating() {
        return this.mCurRotation != this.mOriginalRotation ? true : DEBUG_TRANSFORMS;
    }

    private boolean hasAnimations() {
        return (this.mRotateEnterAnimation == null && this.mRotateExitAnimation == null) ? DEBUG_TRANSFORMS : true;
    }

    private boolean stepAnimation(long now) {
        boolean more = DEBUG_TRANSFORMS;
