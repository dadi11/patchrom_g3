...
public class PermissionMonitor {
    ...
    /* renamed from: com.android.server.connectivity.PermissionMonitor.1 */
    class C01691 extends BroadcastReceiver {
        C01691() {
        }
    ...
    public PermissionMonitor(Context context, INetworkManagementService netd) {
        ...
        this.mNetd = netd;
        this.mIntentReceiver = new C01691();
    }
    ...
    public synchronized void startMonitoring() {
        ...
        this.mContext.registerReceiverAsUser(this.mIntentReceiver, UserHandle.ALL, intentFilter, null, null);
        List<PackageInfo> apps = this.mPackageManager.getInstalledPackages(DumpState.DUMP_VERSION);
        if (apps == null) {
            loge("No apps");
        } else {
    ...
    private boolean hasSystemPermission(PackageInfo app) {
        ...
        if ((flags & 1) == 0 && (flags & DumpState.DUMP_PROVIDERS) == 0) {
    ...
    private void update(Set<Integer> users, Map<Integer, Boolean> apps, boolean add) {
        ...
        for (Entry<Integer, Boolean> app : apps.entrySet()) {
            List<Integer> list;
            if (((Boolean) app.getValue()).booleanValue()) {
                list = system;
            } else {
                list = network;
            }
    ...
    private synchronized void onAppAdded(String appName, int appUid) {
        if (TextUtils.isEmpty(appName) || appUid < 0) {
            ...
            try {
                PackageInfo app = this.mPackageManager.getPackageInfo(appName, DumpState.DUMP_VERSION);
                boolean isNetwork = hasNetworkPermission(app);
                boolean isSystem = hasSystemPermission(app);
                if (isNetwork || isSystem) {
