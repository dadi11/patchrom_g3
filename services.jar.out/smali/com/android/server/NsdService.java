...
public class NsdService extends Stub {
    ...
    private class NsdStateMachine extends StateMachine {
        ...
        class C00751 extends ContentObserver {
            C00751(Handler x0) {
                super(x0);
            }
        ...
        private void registerForNsdSetting() {
            NsdService.this.mContext.getContentResolver().registerContentObserver(Global.getUriFor("nsd_on"), NsdService.DBG, new C00751(getHandler()));
        }
    ...
    private boolean registerService(int regId, NsdServiceInfo service) {
        try {
            Command cmd = new Command("mdnssd", "register", Integer.valueOf(regId), service.getServiceName(), service.getServiceType(), Integer.valueOf(service.getPort()));
            Map<String, byte[]> txtRecords = service.getAttributes();
            for (String key : txtRecords.keySet()) {
                try {
                    byte[] recordValue = (byte[]) txtRecords.get(key);
                    Locale locale = Locale.US;
                    String str = "%s=%s";
                    Object[] objArr = new Object[2];
                    objArr[0] = key;
                    objArr[1] = recordValue != null ? new String(recordValue, "UTF_8") : "";
                    cmd.appendArg(String.format(locale, str, objArr));
