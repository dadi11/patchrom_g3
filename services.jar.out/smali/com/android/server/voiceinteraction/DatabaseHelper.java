...
public class DatabaseHelper extends SQLiteOpenHelper {
                            ...
                            if (modelUuid == null) {
                                ...
                                if (users == null) {
                                    ...
                                    if (isAvailableForCurrentUser) {
                                        KeyphraseSoundModel keyphraseSoundModel2 = new KeyphraseSoundModel(UUID.fromString(modelUuid), null, data, new Keyphrase[]{new Keyphrase(keyphraseId, recognitionModes, modelLocale, text, users)});
                                        c.close();
                                        db.close();
                                    }
                                }
    ...
    private static int[] getArrayForCommaSeparatedString(String text) {
        ...
        for (int i = 0; i < usersStr.length; i++) {
            users[i] = Integer.valueOf(usersStr[i]).intValue();
        }
        return users;
    }
}
