...
public class JobStatus {
    ...
    public JobStatus(JobInfo job, int uId) {
        ...
        this.earliestRunTimeElapsedMillis = job.hasEarlyConstraint() ? job.getMinLatencyMillis() + elapsedNow : NO_EARLIEST_RUNTIME;
    ...
    public boolean hasTimingDelayConstraint() {
        return this.earliestRunTimeElapsedMillis != NO_EARLIEST_RUNTIME;
    }
    ...
    public synchronized boolean isReady() {
        boolean z;
        z = isConstraintsSatisfied() || (hasDeadlineConstraint() && this.deadlineConstraintSatisfied.get());
        return z;
    }

    public synchronized boolean isConstraintsSatisfied() {
        boolean z;
        z = (!hasChargingConstraint() || this.chargingConstraintSatisfied.get()) && ((!hasTimingDelayConstraint() || this.timeDelayConstraintSatisfied.get()) && ((!hasConnectivityConstraint() || this.connectivityConstraintSatisfied.get()) && ((!hasUnmeteredConstraint() || this.unmeteredConstraintSatisfied.get()) && (!hasIdleConstraint() || this.idleConstraintSatisfied.get()))));
        return z;
    }
    ...
    public String toString() {
        return String.valueOf(hashCode()).substring(0, 3) + ".." + ":[" + this.job.getService() + ",jId=" + this.job.getId() + ",u" + getUserId() + ",R=(" + formatRunTime(this.earliestRunTimeElapsedMillis, NO_EARLIEST_RUNTIME) + "," + formatRunTime(this.latestRunTimeElapsedMillis, NO_LATEST_RUNTIME) + ")" + ",N=" + this.job.getNetworkType() + ",C=" + this.job.isRequireCharging() + ",I=" + this.job.isRequireDeviceIdle() + ",F=" + this.numFailures + ",P=" + this.job.isPersisted() + (isReady() ? "(READY)" : "") + "]";
    }
