...
public class BatteryController extends StateController {
    ...
    public class ChargingTracker extends BroadcastReceiver {
        ...
        public ChargingTracker() {
            ...
            this.mStableChargingTriggerIntent = PendingIntent.getBroadcast(BatteryController.this.mContext, 0, new Intent(BatteryController.ACTION_CHARGING_STABLE), 0);
        }
        ...
        void setStableChargingAlarm() {
            this.mAlarm.set(2, SystemClock.elapsedRealtime() + BatteryController.STABLE_CHARGING_THRESHOLD_MILLIS, this.mStableChargingTriggerIntent);
        }
    }
    ...
    private void maybeReportNewChargingState() {
        ...
        synchronized (this.mTrackedTasks) {
            for (JobStatus ts : this.mTrackedTasks) {
                if (ts.chargingConstraintSatisfied.getAndSet(stablePower) != stablePower) {
                    reportChange = true;
                }
            }
        }
