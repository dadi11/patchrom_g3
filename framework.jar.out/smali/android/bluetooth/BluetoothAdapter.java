...
public final class BluetoothAdapter {
    ...
    /* renamed from: android.bluetooth.BluetoothAdapter.1 */
    class C00361 extends Stub {
        C00361() {
        }
    ...
    class C00372 extends ScanCallback {
        final /* synthetic */ LeScanCallback val$callback;
        final /* synthetic */ UUID[] val$serviceUuids;

        C00372(UUID[] uuidArr, LeScanCallback leScanCallback) {
        ...
        public void onScanResult(int callbackType, ScanResult result) {
            ...
            if (scanRecord != null) {
                if (this.val$serviceUuids != null) {
                    ...
                    UUID[] arr$ = this.val$serviceUuids;
                    int len$ = arr$.length;
                    for (int i$ = BluetoothAdapter.STATE_DISCONNECTED; i$ < len$; i$ += BluetoothAdapter.STATE_CONNECTING) {
                        uuids.add(new ParcelUuid(arr$[i$]));
                    }
    ...
    BluetoothAdapter(IBluetoothManager managerService) {
        ...
        this.mManagerCallback = new C00361();
    ...
    public BluetoothServerSocket listenUsingL2capWithServiceRecord(String name, UUID uuid) throws IOException {
        return createNewL2capSocketAndRecord(name, uuid, DBG, DBG);
    }
    ...
    public BluetoothServerSocket listenUsingInsecureL2capWithServiceRecord(String name, UUID uuid) throws IOException {
    ...
    private BluetoothServerSocket createNewL2capSocketAndRecord(String name, UUID uuid, boolean auth, boolean encrypt) throws IOException {
        BluetoothServerSocket socket = new BluetoothServerSocket((int) STATE_DISCONNECTING, auth, encrypt, new ParcelUuid(uuid));
        socket.setServiceName(name);
        int errno = socket.mSocket.bindListen();
        if (errno == 0) {
            return socket;
    ...
    public boolean getProfileProxy(Context context, ServiceListener listener, int profile) {
        ...
        if (profile == STATE_CONNECTING) {
            ...
            return DBG;
        } else if (profile == SCAN_MODE_CONNECTABLE) {
            BluetoothDun dun = new BluetoothDun(context, listener);
            return DBG;
        } else if (profile == SCAN_MODE_NONE) {
            ...
            return DBG;
        } else if (profile != ADDRESS_LENGTH) {
            return false;
        } else {
        ...
        if (proxy != null) {
            switch (profile) {
                ...
                case RelativeLayout.START_OF /*16*/:
                    ...
                    return;
                case ADDRESS_LENGTH /*17*/:
                    ((BluetoothHidDevice) proxy).close();
                    return;
                case SCAN_MODE_NONE /*20*/:
                    break;
                case SCAN_MODE_CONNECTABLE /*21*/:
    ...
    public boolean startLeScan(UUID[] serviceUuids, LeScanCallback callback) {
        ...
        synchronized (this.mLeScanClients) {
            ...
            try {
                ...
                ScanCallback scanCallback = new C00372(serviceUuids, callback);
