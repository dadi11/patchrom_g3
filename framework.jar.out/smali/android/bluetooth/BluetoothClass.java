...
public final class BluetoothClass implements Parcelable {
    ...
    /* renamed from: android.bluetooth.BluetoothClass.1 */
    static class C00411 implements Creator<BluetoothClass> {
        C00411() {
        }
    ...
    public boolean doesClassMatch(int profile) {
        if (profile == PROFILE_A2DP) {
            if (hasService(AccessibilityNodeInfo.ACTION_EXPAND)) {
                return true;
            }
            ...
            if (hasService(AccessibilityNodeInfo.ACTION_COLLAPSE)) {
                return true;
            }
            switch (getDeviceClass()) {
            ...
            if (hasService(AccessibilityNodeInfo.ACTION_DISMISS)) {
                return true;
            }
            switch (getDeviceClass()) {
            ...
            if (hasService(AccessibilityNodeInfo.ACTION_SET_SELECTION) || (getDeviceClass() & GLES20.GL_SRC_COLOR) == GLES20.GL_SRC_COLOR) {
                return true;
            }
            return false;
        }
