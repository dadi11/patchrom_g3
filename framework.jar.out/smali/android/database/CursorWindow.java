...
public class CursorWindow extends SQLiteClosable implements Parcelable {
    ...
    static class C01581 implements Creator<CursorWindow> {
        C01581() {
        }

        public CursorWindow createFromParcel(Parcel source) {
            return new CursorWindow(null);
        }
    ...
    public CursorWindow(String name) {
        ...
        if (sCursorWindowSize < 0) {
            sCursorWindowSize = Resources.getSystem().getInteger(17694826) * AccessibilityNodeInfo.ACTION_NEXT_HTML_ELEMENT;
        }
        this.mWindowPtr = nativeCreate(this.mName, sCursorWindowSize);
        if (this.mWindowPtr == 0) {
            throw new CursorWindowAllocationException("Cursor window allocation of " + (sCursorWindowSize / AccessibilityNodeInfo.ACTION_NEXT_HTML_ELEMENT) + " kb failed. " + printStats());
        }
