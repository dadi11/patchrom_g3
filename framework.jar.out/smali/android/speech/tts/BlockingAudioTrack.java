...
class BlockingAudioTrack {
    ...
    long getAudioLengthMs(int numBytes) {
        return (long) (((numBytes / this.mBytesPerFrame) * LayoutParams.TYPE_APPLICATION_PANEL) / this.mSampleRateInHz);
    }
    ...
    private void blockUntilEstimatedCompletion() {
        try {
            Thread.sleep((long) (((this.mBytesWritten / this.mBytesPerFrame) * LayoutParams.TYPE_APPLICATION_PANEL) / this.mSampleRateInHz));
    ...
    private void blockUntilCompletion(AudioTrack audioTrack) {
        ...
        while (true) {
            ...
            if (currentPosition < lengthInFrames && audioTrack.getPlayState() == 3 && !this.mStopped) {
                long sleepTimeMs = clip((long) (((lengthInFrames - currentPosition) * LayoutParams.TYPE_APPLICATION_PANEL) / audioTrack.getSampleRate()), (long) MIN_SLEEP_TIME_MS, (long) MAX_SLEEP_TIME_MS);
    ...
    private static void setupVolume(AudioTrack audioTrack, float volume, float pan) {
        float vol = clip(volume, 0.0f, (float) LayoutParams.BRIGHTNESS_OVERRIDE_FULL);
        float panning = clip(pan, (float) LayoutParams.BRIGHTNESS_OVERRIDE_NONE, (float) LayoutParams.BRIGHTNESS_OVERRIDE_FULL);
        float volLeft = vol;
        float volRight = vol;
        if (panning > 0.0f) {
