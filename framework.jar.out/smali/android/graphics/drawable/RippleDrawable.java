...
public class RippleDrawable extends LayerDrawable {
    ...
    public RippleDrawable(ColorStateList color, Drawable content, Drawable mask) {
        ...
        if (mask != null) {
            addLayer(mask, null, C0000R.id.mask, MASK_NONE, MASK_NONE, MASK_NONE, MASK_NONE);
        }
        setColor(color);
        ensurePadding();
        initializeFromState();
    ...
    public void getOutline(Outline outline) {
        ...
        for (int i = MASK_NONE; i < N; i += MASK_CONTENT) {
            if (children[i].mId != C0000R.id.mask) {
    ...
    private void drawBackgroundAndRipples(Canvas canvas) {
        ...
        if (active != null || count > 0 || (background != null && background.shouldDraw())) {
            ...
            int color = this.mState.mColor.getColorForState(getState(), Spanned.SPAN_USER);
            int halfAlpha = (Color.alpha(color) / MASK_EXPLICIT) << 24;
            Paint p = getRipplePaint();
            if (this.mMaskColorFilter != null) {
                this.mMaskColorFilter.setColor(color | Spanned.SPAN_USER);
    ...
    public Drawable mutate() {
        ...
        this.mMask = findDrawableByLayerId(C0000R.id.mask);
        return this;
    }
