...
public final class Bitmap implements Parcelable {
    ...
    static class C01731 implements Creator<Bitmap> {
        C01731() {
        }
    ...
    public static Bitmap createBitmap(Bitmap source, int x, int y, int width, int height, Matrix m, boolean filter) {
        ...
        if (x + width > source.getWidth()) {
            ...
            if (config != null) {
                switch (C01742.$SwitchMap$android$graphics$Bitmap$Config[config.ordinal()]) {
    ...
    private static Bitmap createBitmap(DisplayMetrics display, int width, int height, Config config, boolean hasAlpha) {
        ...
        if (config == Config.ARGB_8888 && !hasAlpha) {
            nativeErase(bm.mNativeBitmap, Spanned.SPAN_USER);
        }
        return bm;
    }
