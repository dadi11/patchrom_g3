...
public class ETC1Util {
    ...
    public static void loadTexture(int target, int level, int border, int fallbackFormat, int fallbackType, ETC1Texture texture) {
        if (fallbackFormat != 6407) {
            ...
            if (isETC1Supported()) {
                GLES10.glCompressedTexImage2D(target, level, GLES11Ext.GL_ETC1_RGB8_OES, width, height, border, data.remaining(), data);
                return;
            }
    ...
    public static boolean isETC1Supported() {
        ...
        GLES10.glGetIntegerv(GLES20.GL_NUM_COMPRESSED_TEXTURE_FORMATS, results, 0);
        int numFormats = results[0];
        if (numFormats > results.length) {
            results = new int[numFormats];
        }
        GLES10.glGetIntegerv(GLES20.GL_COMPRESSED_TEXTURE_FORMATS, results, 0);
    ...
    public static ETC1Texture createTexture(InputStream input) throws IOException {
        byte[] ioBuffer = new byte[AccessibilityNodeInfo.ACTION_SCROLL_FORWARD];
