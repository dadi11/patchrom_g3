...
public class DnsSdTxtRecord implements Parcelable {
    ...
    /* renamed from: android.net.nsd.DnsSdTxtRecord.1 */
    static class C05181 implements Creator<DnsSdTxtRecord> {
        C05181() {
        }
    ...
    public void set(String key, String value) {
        ...
        try {
            ...
            if (keyBytes.length + valLen >= EditorInfo.IME_MASK_ACTION) {
    ...
    public int remove(String key) {
        ...
        while (avStart < this.mData.length) {
            ...
            if (key.length() > avLen || !((key.length() == avLen || this.mData[(key.length() + avStart) + 1] == 61) && key.compareToIgnoreCase(new String(this.mData, avStart + 1, key.length())) == 0)) {
                avStart += (avLen + 1) & EditorInfo.IME_MASK_ACTION;
                i++;
            } else {
    ...
    public int keyCount() {
        ...
        while (nextKey < this.mData.length) {
            nextKey += (this.mData[nextKey] + 1) & EditorInfo.IME_MASK_ACTION;
            count++;
        }
        return count;
    }
    ...
    private void insert(byte[] keyBytes, byte[] value, int index) {
        ...
        for (int i2 = 0; i2 < index && insertion < this.mData.length; i2++) {
            insertion += (this.mData[insertion] + 1) & EditorInfo.IME_MASK_ACTION;
        }
