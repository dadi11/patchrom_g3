...
public class SSLCertificateSocketFactory extends SSLSocketFactory {
    ...
    static class C05051 implements X509TrustManager {
        C05051() {
        }
    ...
    static {
        INSECURE_TRUST_MANAGER = new TrustManager[]{new C05051()};
    }
    ...
    private static boolean isSslCheckRelaxed() {
        return WifiEnterpriseConfig.ENGINE_ENABLE.equals(SystemProperties.get("ro.debuggable")) && "yes".equals(SystemProperties.get("socket.relaxsslcheck"));
    }
    ...
    static byte[] toLengthPrefixedList(byte[]... items) {
        ...
        int totalLength = 0;
        for (byte[] s : items) {
            if (s.length == 0 || s.length > EditorInfo.IME_MASK_ACTION) {
