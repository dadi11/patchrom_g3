...
public class CertificateChainValidator {
    ...
    public static void handleTrustStorageUpdate() {
        try {
            ...
            Slog.w(TAG, "Couldn't initialize default X.509 TrustManagerFactory", e3);
        }
    }
    ...
    private static SslError verifyServerDomainAndCertificates(X509Certificate[] chain, String domain, String authType) throws IOException {
        ...
        if (!(domain == null || domain.isEmpty() || !NoPreloadHolder.sVerifier.verify(domain, new CertificateWrap(currCertificate)))) {
            valid = true;
        }
        if (!valid) {
        ...
        try {
            ...
            if (x509TrustManager instanceof TrustManagerImpl) {
                ((TrustManagerImpl) x509TrustManager).checkServerTrusted(chain, authType, domain);
            } else {
