...
public class SslCertificate {
    ...
    private static final String fingerprint(byte[] bytes) {
        ...
        for (int i = 0; i < bytes.length; i++) {
            IntegralToString.appendByteAsHex(sb, bytes[i], true);
            if (i + 1 != bytes.length) {
                sb.append(':');
            }
        }
    ...
    public View inflateCertificateView(Context context) {
        View certificateView = LayoutInflater.from(context).inflate(17367248, null);
        DName issuedTo = getIssuedTo();
        if (issuedTo != null) {
            ((TextView) certificateView.findViewById(16909211)).setText(issuedTo.getCName());
            ((TextView) certificateView.findViewById(16909213)).setText(issuedTo.getOName());
            ((TextView) certificateView.findViewById(16909215)).setText(issuedTo.getUName());
        }
        ((TextView) certificateView.findViewById(16909217)).setText(getSerialNumber(this.mX509Certificate));
        DName issuedBy = getIssuedBy();
        if (issuedBy != null) {
            ((TextView) certificateView.findViewById(16909219)).setText(issuedBy.getCName());
            ((TextView) certificateView.findViewById(16909221)).setText(issuedBy.getOName());
            ((TextView) certificateView.findViewById(16909223)).setText(issuedBy.getUName());
        }
        ((TextView) certificateView.findViewById(16909226)).setText(formatCertificateDate(context, getValidNotBeforeDate()));
        ((TextView) certificateView.findViewById(16909228)).setText(formatCertificateDate(context, getValidNotAfterDate()));
        ((TextView) certificateView.findViewById(16909231)).setText(getDigest(this.mX509Certificate, "SHA256"));
        ((TextView) certificateView.findViewById(16909233)).setText(getDigest(this.mX509Certificate, "SHA1"));
