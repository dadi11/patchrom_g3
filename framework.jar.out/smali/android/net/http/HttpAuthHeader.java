...
public class HttpAuthHeader {
    ...
    public boolean isBasic() {
        return this.mScheme == BASIC;
    }

    public boolean isDigest() {
        return this.mScheme == DIGEST;
    }
    ...
    public boolean isSupportedScheme() {
        if (this.mRealm != null) {
            if (this.mScheme == BASIC) {
                return true;
            }
            if (this.mScheme == DIGEST) {
        ...
        if (header != null) {
            ...
            if (i >= 0) {
                ...
                if (scheme.equalsIgnoreCase(DIGEST_TOKEN)) {
                    this.mScheme = DIGEST;
                    this.mAlgorithm = "md5";
                } else if (scheme.equalsIgnoreCase(BASIC_TOKEN)) {
                    this.mScheme = BASIC;
                }
                return header.substring(i + BASIC);
            }
        }
        return null;
    }
        ...
        if (parameters != null) {
            ...
            do {
                ...
                if (i < 0) {
                    ...
                    parameters = parameters.substring(i + BASIC);
                    continue;
                }
            } while (i >= 0);
        }
    ...
    private void parseParameter(String parameter) {
        if (parameter != null) {
            ...
            if (i >= 0) {
                ...
                String value = trimDoubleQuotesIfAny(parameter.substring(i + BASIC).trim());
                if (token.equalsIgnoreCase(REALM_TOKEN)) {
                    this.mRealm = value;
                } else if (this.mScheme == DIGEST) {
    ...
    private static String trimDoubleQuotesIfAny(String value) {
        ...
        if (len > DIGEST && value.charAt(0) == '\"' && value.charAt(len - 1) == '\"') {
            return value.substring(BASIC, len - 1);
        }
        return value;
    }
}
