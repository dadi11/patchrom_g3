...
public class Location implements Parcelable {
    ...
    static class C03471 implements Creator<Location> {
        C03471() {
        }
    ...
    public Location(String provider) {
        ...
        this.mResults = new float[FORMAT_SECONDS];
    ...
    public static double convert(String coordinate) {
        ...
        try {
            ...
            boolean isNegative180 = negative && deg == KeyEvent.KEYCODE_STB_INPUT && min == 0.0d && sec == 0.0d;
            if (((double) deg) < 0.0d || (deg > KeyEvent.KEYCODE_STB_POWER && !isNegative180)) {
                throw new IllegalArgumentException("coordinate=" + coordinate);
    ...
    private static void computeDistanceAndBearing(double lat1, double lon1, double lat2, double lon2, float[] results) {
        ...
        results[FORMAT_DEGREES] = (float) ((6356752.3142d * A) * (sigma - deltaSigma));
        int length = results.length;
        if (r0 > FORMAT_MINUTES) {
            results[FORMAT_MINUTES] = (float) (((double) ((float) Math.atan2(cosU2 * sinLambda, (cosU1 * sinU2) - ((sinU1 * cosU2) * cosLambda)))) * 57.29577951308232d);
            length = results.length;
            if (r0 > FORMAT_SECONDS) {
                results[FORMAT_SECONDS] = (float) (((double) ((float) Math.atan2(cosU1 * sinLambda, ((-sinU1) * cosU2) + ((cosU1 * sinU2) * cosLambda)))) * 57.29577951308232d);
            }
        }
    }
    ...
    public float distanceTo(Location dest) {
        ...
        synchronized (this.mResults) {
            if (!(this.mLatitude == this.mLat1 && this.mLongitude == this.mLon1 && dest.mLatitude == this.mLat2 && dest.mLongitude == this.mLon2)) {
                ...
                this.mDistance = this.mResults[FORMAT_DEGREES];
                this.mInitialBearing = this.mResults[FORMAT_MINUTES];
            }
            f = this.mDistance;
        }
        return f;
    ...
    public String toString() {
        ...
        s.append(String.format(" %.6f,%.6f", objArr));
        if (this.mHasAccuracy) {
    ...
    public void writeToParcel(Parcel parcel, int flags) {
        int i;
        int i2 = FORMAT_MINUTES;
        parcel.writeString(this.mProvider);
        parcel.writeLong(this.mTime);
        parcel.writeLong(this.mElapsedRealtimeNanos);
        parcel.writeDouble(this.mLatitude);
        parcel.writeDouble(this.mLongitude);
        parcel.writeInt(this.mHasAltitude ? FORMAT_MINUTES : FORMAT_DEGREES);
