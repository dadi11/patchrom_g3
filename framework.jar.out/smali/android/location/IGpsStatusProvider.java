...
public interface IGpsStatusProvider extends IInterface {
    ...
    public static abstract class Stub extends Binder implements IGpsStatusProvider {
        ...
        public boolean onTransact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
            switch (code) {
                case TRANSACTION_addGpsStatusListener /*1*/:
                    data.enforceInterface(DESCRIPTOR);
                    addGpsStatusListener(android.location.IGpsStatusListener.Stub.asInterface(data.readStrongBinder()));
                    reply.writeNoException();
                    return true;
                case TRANSACTION_removeGpsStatusListener /*2*/:
                    data.enforceInterface(DESCRIPTOR);
