...
public abstract class MetaKeyKeyListener {
    ...
    public static long resetLockedMeta(long state) {
        ...
        if ((Trace.TRACE_TAG_CAMERA & state) != 0) {
    ...
    public static long handleKeyDown(long state, int keyCode, KeyEvent event) {
        ...
        if (keyCode == 63) {
            return press(state, META_SYM_ON, META_SYM_MASK, Trace.TRACE_TAG_CAMERA, META_SYM_PRESSED, META_SYM_RELEASED, META_SYM_USED);
        }
        return state;
    }
    ...
    public long clearMetaKeyState(long state, int which) {
        ...
        if ((which & META_SYM_ON) == 0 || (Trace.TRACE_TAG_CAMERA & state) == 0) {
            return state;
        }
        return state & -1130315133223941L;
    }
