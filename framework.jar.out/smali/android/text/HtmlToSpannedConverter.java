...
class HtmlToSpannedConverter implements ContentHandler {
    ...
    static {
        HEADER_SIZES = new float[]{1.5f, 1.4f, 1.3f, 1.2f, 1.1f, LayoutParams.BRIGHTNESS_OVERRIDE_FULL};
    }
    ...
    private static void startImg(SpannableStringBuilder text, Attributes attributes, ImageGetter img) {
        ...
        if (d == null) {
            d = Resources.getSystem().getDrawable(17303390);
    ...
    private static void startFont(SpannableStringBuilder text, Attributes attributes) {
        String color = attributes.getValue(ProxyInfo.LOCAL_EXCL_LIST, SubscriptionManager.COLOR);
        ...
        if (where != len) {
            ...
            if (!TextUtils.isEmpty(f.mColor)) {
                if (f.mColor.startsWith("@")) {
                    ...
                    if (c != -1) {
                        text.setSpan(new ForegroundColorSpan(Spanned.SPAN_USER | c), where, len, 33);
                    }
                }
            }
            if (f.mFace != null) {
