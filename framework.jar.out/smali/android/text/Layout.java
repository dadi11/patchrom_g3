...
public abstract class Layout {
    ...
    static class TabStops {
        ...
        void reset(int increment, Object[] spans) {
            ...
            int ns = Layout.MIN_EMOJI;
            if (spans != null) {
                int[] stops = this.mStops;
                Object[] arr$ = spans;
                int len$ = arr$.length;
                int i$ = Layout.MIN_EMOJI;
                int ns2 = Layout.MIN_EMOJI;
                while (i$ < len$) {
                    Object o = arr$[i$];
                    if (o instanceof TabStopSpan) {
                        if (stops == null) {
                            ...
                            int[] nstops = new int[(ns2 * Layout.DIR_REQUEST_DEFAULT_LTR)];
                            for (int i = Layout.MIN_EMOJI; i < ns2; i += Layout.DIR_REQUEST_LTR) {
                                nstops[i] = stops[i];
                            }
                            stops = nstops;
                        }
                        ns = ns2 + Layout.DIR_REQUEST_LTR;
                        stops[ns2] = ((TabStopSpan) o).getTabStop();
                    } else {
                        ns = ns2;
                    }
                    i$ += Layout.DIR_REQUEST_LTR;
                    ns2 = ns;
                }
                if (ns2 > Layout.DIR_REQUEST_LTR) {
                    Arrays.sort(stops, Layout.MIN_EMOJI, ns2);
                }
        ...
        float nextTab(float h) {
            ...
            if (ns > 0) {
                int[] stops = this.mStops;
                for (int i = Layout.MIN_EMOJI; i < ns; i += Layout.DIR_REQUEST_LTR) {
                    int stop = stops[i];
                    if (((float) stop) > h) {
                        return (float) stop;
                    }
    ...
    static {
        NO_PARA_SPANS = (ParagraphStyle[]) ArrayUtils.emptyArray(ParagraphStyle.class);
        ...
        if (EMOJI_FACTORY != null) {
            ...
            MIN_EMOJI = DIR_RIGHT_TO_LEFT;
            MAX_EMOJI = DIR_RIGHT_TO_LEFT;
        }
        sTempRect = new Rect();
        DIRS_ALL_LEFT_TO_RIGHT = new Directions(new int[]{MIN_EMOJI, RUN_LENGTH_MASK});
        DIRS_ALL_RIGHT_TO_LEFT = new Directions(new int[]{MIN_EMOJI, 134217727});
    }
                        ...
                        if (start != end || start == 0) {
                            ...
                            while (j < this.mLineBackgroundSpans.numberOfSpans) {
                                if (this.mLineBackgroundSpans.spanStarts[j] < end && this.mLineBackgroundSpans.spanEnds[j] > start) {
                                    spans = (ParagraphStyle[]) GrowingArrayUtils.append(spans, spansLength, ((LineBackgroundSpan[]) this.mLineBackgroundSpans.spans)[j]);
    ...
    public boolean shouldClampCursor(int line) {
        switch (C07981.$SwitchMap$android$text$Layout$Alignment[getParagraphAlignment(line).ordinal()]) {
    ...
    public void getCursorPath(int point, Path dest, CharSequence editingBuffer) {
        ...
        int caps = MetaKeyKeyListener.getMetaState(editingBuffer, (int) DIR_REQUEST_LTR) | MetaKeyKeyListener.getMetaState(editingBuffer, (int) AccessibilityNodeInfo.ACTION_PREVIOUS_HTML_ELEMENT);
