...
public class BoringLayout extends Layout implements EllipsizeCallback {
    ...
    public static Metrics isBoring(CharSequence text, TextPaint paint, TextDirectionHeuristic textDir, Metrics metrics) {
        char[] temp = TextUtils.obtain(AccessibilityEvent.MAX_TEXT_LENGTH);
        int length = text.length();
        boolean boring = true;
        loop0:
        for (int i = 0; i < length; i += AccessibilityEvent.MAX_TEXT_LENGTH) {
            int j = i + AccessibilityEvent.MAX_TEXT_LENGTH;
            if (j > length) {
                j = length;
            }
