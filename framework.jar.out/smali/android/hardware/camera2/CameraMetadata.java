...
public abstract class CameraMetadata<TKey> {
    ...
    static <TKey> ArrayList<TKey> getKeysStatic(Class<?> type, Class<TKey> keyClass, CameraMetadata<TKey> instance, int[] filterTags) {
        ...
        int len$ = arr$.length;
        for (int i$ = TONEMAP_MODE_CONTRAST_CURVE; i$ < len$; i$ += TONEMAP_MODE_FAST) {
            Field field = arr$[i$];
            if (field.getType().isAssignableFrom(keyClass) && (field.getModifiers() & TONEMAP_MODE_FAST) != 0) {
                try {
    ...
    private static <TKey> boolean shouldKeyBeAdded(TKey key, Field field, int[] filterTags) {
        ...
        if (filterTags == null || field.getAnnotation(SyntheticKey.class) != null || Arrays.binarySearch(filterTags, nativeKey.getTag()) >= 0) {
            return true;
        }
        return false;
    }
