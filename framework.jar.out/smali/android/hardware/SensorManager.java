...
public abstract class SensorManager {
    ...
    static {
        mTempMatrix = new float[SENSOR_LIGHT];
    }
    ...
    public Sensor getDefaultSensor(int type) {
        ...
        if (type == SENSOR_MAGNETIC_FIELD || type == 17 || type == 22 || type == 23 || type == 24 || type == 25) {
            wakeUpSensor = true;
        }
        for (Sensor sensor : l) {
    ...
    public void unregisterListener(SensorListener listener) {
        unregisterListener(listener, (int) EditorInfo.IME_MASK_ACTION);
    }
    ...
    public static boolean getRotationMatrix(float[] R, float[] I, float[] gravity, float[] geomagnetic) {
        ...
        int length;
        float invH = LayoutParams.BRIGHTNESS_OVERRIDE_FULL / normH;
        Hx *= invH;
        Hy *= invH;
        Hz *= invH;
        float invA = LayoutParams.BRIGHTNESS_OVERRIDE_FULL / ((float) Math.sqrt((double) (((Ax * Ax) + (Ay * Ay)) + (Az * Az))));
        Ax *= invA;
        Ay *= invA;
        Az *= invA;
        ...
        if (R != null) {
            ...
            if (r0 == 9) {
                ...
                if (r0 == SENSOR_LIGHT) {
                    ...
                    R[13] = 0.0f;
                    R[14] = 0.0f;
                    R[15] = LayoutParams.BRIGHTNESS_OVERRIDE_FULL;
                }
            }
        }
        if (I != null) {
            float invE = LayoutParams.BRIGHTNESS_OVERRIDE_FULL / ((float) Math.sqrt((double) (((Ex * Ex) + (Ey * Ey)) + (Ez * Ez))));
            float c = (((Ex * Mx) + (Ey * My)) + (Ez * Mz)) * invE;
            float s = (((Ex * Ax) + (Ey * Ay)) + (Ez * Az)) * invE;
            length = I.length;
            if (r0 == 9) {
                I[SENSOR_STATUS_UNRELIABLE] = LayoutParams.BRIGHTNESS_OVERRIDE_FULL;
                ...
                if (r0 == SENSOR_LIGHT) {
                    ...
                    I[7] = 0.0f;
                    I[SENSOR_STATUS_ACCURACY_HIGH] = 0.0f;
                    I[15] = LayoutParams.BRIGHTNESS_OVERRIDE_FULL;
                }
            }
        }
        return true;
    ...
    private static boolean remapCoordinateSystemImpl(float[] inR, int X, int Y, float[] outR) {
        ...
        if ((X & KeyEvent.KEYCODE_INSERT) != 0 || (Y & KeyEvent.KEYCODE_INSERT) != 0) {
            return false;
        }
        ...
        if (length == SENSOR_LIGHT) {
            ...
            outR[15] = LayoutParams.BRIGHTNESS_OVERRIDE_FULL;
        }
        return true;
    }
    ...
    public static float getAltitude(float p0, float p) {
        return 44330.0f * (LayoutParams.BRIGHTNESS_OVERRIDE_FULL - ((float) Math.pow((double) (p / p0), 0.19029495120048523d)));
    }
    ...
    public static void getRotationMatrixFromVector(float[] R, float[] rotationVector) {
        ...
        if (rotationVector.length >= SENSOR_TEMPERATURE) {
            ...
            q0 = ((LayoutParams.BRIGHTNESS_OVERRIDE_FULL - (q1 * q1)) - (q2 * q2)) - (q3 * q3);
            if (q0 > 0.0f) {
        ...
        if (R.length == 9) {
            R[SENSOR_STATUS_UNRELIABLE] = (LayoutParams.BRIGHTNESS_OVERRIDE_FULL - sq_q2) - sq_q3;
            ...
            R[SENSOR_TEMPERATURE] = (LayoutParams.BRIGHTNESS_OVERRIDE_FULL - sq_q1) - sq_q3;
            R[RAW_DATA_Z] = q2_q3 - q1_q0;
            R[6] = q1_q3 - q2_q0;
            R[7] = q2_q3 + q1_q0;
            R[SENSOR_MAGNETIC_FIELD] = (LayoutParams.BRIGHTNESS_OVERRIDE_FULL - sq_q1) - sq_q2;
            ...
            R[RAW_DATA_Z] = (LayoutParams.BRIGHTNESS_OVERRIDE_FULL - sq_q1) - sq_q3;
            R[6] = q2_q3 - q1_q0;
            R[7] = 0.0f;
            R[SENSOR_MAGNETIC_FIELD] = q1_q3 - q2_q0;
            R[9] = q2_q3 + q1_q0;
            R[10] = (LayoutParams.BRIGHTNESS_OVERRIDE_FULL - sq_q1) - sq_q2;
            R[11] = 0.0f;
            R[14] = 0.0f;
            R[13] = 0.0f;
            R[12] = 0.0f;
    ...
    public static void getQuaternionFromVector(float[] Q, float[] rv) {
        ...
        if (rv.length >= SENSOR_TEMPERATURE) {
            ...
            Q[SENSOR_STATUS_UNRELIABLE] = ((LayoutParams.BRIGHTNESS_OVERRIDE_FULL - (rv[SENSOR_STATUS_UNRELIABLE] * rv[SENSOR_STATUS_UNRELIABLE])) - (rv[SENSOR_STATUS_ACCURACY_LOW] * rv[SENSOR_STATUS_ACCURACY_LOW])) - (rv[SENSOR_STATUS_ACCURACY_MEDIUM] * rv[SENSOR_STATUS_ACCURACY_MEDIUM]);
