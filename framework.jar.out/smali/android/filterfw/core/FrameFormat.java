...
public class FrameFormat {
    ...
    public String toString() {
        ...
        return (this.mTarget == 0 ? ProxyInfo.LOCAL_EXCL_LIST : targetToString(this.mTarget) + " ") + baseTypeToString(this.mBaseType) + (valuesPerSample == TYPE_BIT ? ProxyInfo.LOCAL_EXCL_LIST : String.valueOf(valuesPerSample)) + dimensionsToString(this.mDimensions) + (this.mObjectClass == null ? ProxyInfo.LOCAL_EXCL_LIST : " class(" + this.mObjectClass.getSimpleName() + ") ") + metaDataToString(this.mMetaData);
    }
    ...
    int calcSize(int[] dimensions) {
        ...
        int[] arr$ = dimensions;
        int len$ = arr$.length;
        for (int i$ = TARGET_UNSPECIFIED; i$ < len$; i$ += TYPE_BIT) {
            size *= arr$[i$];
        }
        return size;
    }
