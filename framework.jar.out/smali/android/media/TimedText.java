...
public final class TimedText {
        ...
        while (!endOfStyle && parcel.dataAvail() > 0) {
            switch (parcel.readInt()) {
                case KEY_STYLE_FLAGS /*2*/:
                    ...
                    isUnderlined = flags / KEY_HIGHLIGHT_COLOR_RGBA == KEY_DISPLAY_FLAGS;
                    break;
    ...
    private boolean isValidKey(int key) {
        if (key >= KEY_DISPLAY_FLAGS && key <= LAST_PUBLIC_KEY) {
            return true;
        }
        if (key < KEY_GLOBAL_SETTING || key > LAST_PRIVATE_KEY) {
            return false;
        }
        return true;
    }
