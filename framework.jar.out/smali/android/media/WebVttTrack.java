...
class WebVttTrack extends SubtitleTrack implements WebVttCueListener {
    ...
    public void onCueParsed(TextTrackCue cue) {
        synchronized (this.mParser) {
            ...
            for (TextTrackCueSpan[] arr$ : cue.mLines) {
                for (TextTrackCueSpan span : r2[r5]) {
                    if (span.mTimestampMs > cue.mStartTimeMs) {
                        if (span.mTimestampMs < cue.mEndTimeMs && !this.mTimestamps.contains(Long.valueOf(span.mTimestampMs))) {
