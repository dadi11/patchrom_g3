...
public class EffectFactory {
    ...
    static {
        EFFECT_PACKAGES = new String[]{"android.media.effect.effects.", ProxyInfo.LOCAL_EXCL_LIST};
    }
    ...
    private static Class getEffectClassByName(String className) {
        ...
        String[] arr$ = EFFECT_PACKAGES;
        int len$ = arr$.length;
        int i$ = 0;
        while (i$ < len$) {
            try {
                effectClass = contextClassLoader.loadClass(arr$[i$] + className);
                if (effectClass != null) {
                    break;
                }
                i$++;
