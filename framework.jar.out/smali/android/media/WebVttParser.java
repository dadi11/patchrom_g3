...
class WebVttParser {
    ...
    /* renamed from: android.media.WebVttParser.1 */
    class WebVttRenderer implements Phase {
        WebVttRenderer() {
        }
    ...
    /* renamed from: android.media.WebVttParser.2 */
    class WebVttRenderer implements Phase {
        WebVttRenderer() {
        }
            ...
            for (String setting : s.split(" +")) {
                ...
                if (equalAt > 0 && equalAt != setting.length() - 1) {
                    ...
                    if (name.equals("id")) {
                        ...
                        if (value.equals("up")) {
                            region.mScrollValue = MediaFile.FILE_TYPE_3GPA;
                        } else {
    ...
    class WebVttRenderer implements Phase {
        ...
        public void parse(String line) {
            ...
            String rest = line.substring(arrowAt + 3).replaceFirst("^\\s+", ProxyInfo.LOCAL_EXCL_LIST).replaceFirst("\\s+", " ");
            int spaceAt = rest.indexOf(32);
            if (spaceAt > 0) {
                end = rest.substring(0, spaceAt);
            } else {
                end = rest;
            }
            ...
            for (String setting : rest.split(" +")) {
                ...
                if (colonAt > 0 && colonAt != setting.length() - 1) {
                    ...
                    if (name.equals(TtmlUtils.TAG_REGION)) {
                        ...
                        if (value.equals("rl")) {
                            WebVttParser.this.mCue.mWritingDirection = KeyEvent.KEYCODE_BUTTON_Z;
                        } else if (value.equals("lr")) {
                            WebVttParser.this.mCue.mWritingDirection = KeyEvent.KEYCODE_BUTTON_L1;
                        } else {
                        ...
                        if (value.equals("start")) {
                            WebVttParser.this.mCue.mAlignment = KeyEvent.KEYCODE_BUTTON_14;
                        } else if (value.equals("middle")) {
                            WebVttParser.this.mCue.mAlignment = KeyEvent.KEYCODE_BUTTON_13;
                        } else if (value.equals(TtmlUtils.ATTR_END)) {
                            WebVttParser.this.mCue.mAlignment = KeyEvent.KEYCODE_BUTTON_15;
                        } else if (value.equals("left")) {
                            WebVttParser.this.mCue.mAlignment = KeyEvent.KEYCODE_BUTTON_16;
                        } else if (value.equals("right")) {
                            WebVttParser.this.mCue.mAlignment = KeyEvent.KEYCODE_LANGUAGE_SWITCH;
                        } else {
                            WebVttParser.this.log_warning("cue setting", name, "has invalid value", value);
                        }
                    }
    ...
    WebVttParser(WebVttCueListener listener) {
        this.mSkipRest = new WebVttRenderer();
        this.mParseStart = new WebVttRenderer();
        this.mParseHeader = new WebVttRenderer();
        this.mParseCueId = new WebVttRenderer();
        this.mParseCueTime = new WebVttRenderer();
        this.mParseCueText = new WebVttRenderer();
    ...
    public static String timeToString(long timeMs) {
        return String.format("%d:%02d:%02d.%03d", new Object[]{Long.valueOf(timeMs / DateUtils.HOUR_IN_MILLIS), Long.valueOf((timeMs / DateUtils.MINUTE_IN_MILLIS) % 60), Long.valueOf((timeMs / 1000) % 60), Long.valueOf(timeMs % 1000)});
    }
