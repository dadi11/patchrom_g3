...
public class Virtualizer extends AudioEffect {
    ...
    public boolean forceVirtualizationMode(int virtualizationMode) throws IllegalStateException, IllegalArgumentException, UnsupportedOperationException {
        int status = setParameter((int) VIRTUALIZATION_MODE_TRANSAURAL, AudioDevice.convertDeviceTypeToInternalDevice(getDeviceForModeForce(virtualizationMode)));
        if (status >= 0) {
            return true;
        }
        if (status == -4) {
    ...
    public int getVirtualizationMode() throws IllegalStateException, UnsupportedOperationException {
        ...
        if (status >= 0) {
            return deviceToMode(AudioDevice.convertInternalDeviceToDeviceType(value[VIRTUALIZATION_MODE_OFF]));
        }
        if (status == -4) {
            return VIRTUALIZATION_MODE_OFF;
        }
