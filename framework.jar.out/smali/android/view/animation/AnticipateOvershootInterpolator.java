...
public class AnticipateOvershootInterpolator extends BaseInterpolator implements NativeInterpolatorFactory {
    ...
    private static float m27a(float t, float s) {
        return (t * t) * (((LayoutParams.BRIGHTNESS_OVERRIDE_FULL + s) * t) - s);
    }

    private static float m28o(float t, float s) {
        return (t * t) * (((LayoutParams.BRIGHTNESS_OVERRIDE_FULL + s) * t) + s);
    }

    public float getInterpolation(float t) {
        if (t < 0.5f) {
            return m27a(t * 2.0f, this.mTension) * 0.5f;
        }
        return (m28o((t * 2.0f) - 2.0f, this.mTension) + 2.0f) * 0.5f;
    }
