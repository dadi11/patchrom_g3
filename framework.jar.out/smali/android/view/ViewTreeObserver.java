...
public final class ViewTreeObserver {
    ...
    public static final class InternalInsetsInfo {
        ...
        void reset() {
            ...
            this.mTouchableInsets = TOUCHABLE_INSETS_FRAME;
        }
    ...
    final void dispatchOnWindowAttachedChange(boolean attached) {
        ...
        if (listeners != null && listeners.size() > 0) {
            ...
            while (i$.hasNext()) {
                OnWindowAttachListener listener = (OnWindowAttachListener) i$.next();
                if (attached) {
    ...
    final void dispatchOnWindowFocusChange(boolean hasFocus) {
        ...
        if (listeners != null && listeners.size() > 0) {
            Iterator i$ = listeners.iterator();
            while (i$.hasNext()) {
                ((OnWindowFocusChangeListener) i$.next()).onWindowFocusChanged(hasFocus);
            }
        }
    }
    ...
    final void dispatchOnGlobalFocusChange(View oldFocus, View newFocus) {
        ...
        if (listeners != null && listeners.size() > 0) {
            Iterator i$ = listeners.iterator();
            while (i$.hasNext()) {
                ((OnGlobalFocusChangeListener) i$.next()).onGlobalFocusChanged(oldFocus, newFocus);
            }
        }
    }
        ...
        if (listeners != null && listeners.size() > 0) {
            ...
            try {
                for (int i = 0; i < access.size(); i++) {
                    cancelDraw |= !((OnPreDrawListener) access.get(i)).onPreDraw() ? 1 : 0;
                }
            } finally {
                listeners.end();
            }
    ...
    final void dispatchOnTouchModeChanged(boolean inTouchMode) {
        ...
        if (listeners != null && listeners.size() > 0) {
            Iterator i$ = listeners.iterator();
            while (i$.hasNext()) {
                ((OnTouchModeChangeListener) i$.next()).onTouchModeChanged(inTouchMode);
            }
        }
    }
    ...
    public final void dispatchOnEnterAnimationComplete() {
        ...
        if (listeners != null && !listeners.isEmpty()) {
            Iterator i$ = listeners.iterator();
            while (i$.hasNext()) {
                ((OnEnterAnimationCompleteListener) i$.next()).onEnterAnimationComplete();
            }
        }
    }
}
