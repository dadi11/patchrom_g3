...
public class UndoManager {
    ...
    private final HashMap<String, UndoOwner> mOwners;
    ...
    static final class UndoState {
        ...
        <T extends UndoOperation> T getLastOperation(Class<T> clazz, UndoOwner owner) {
            ...
            if (clazz != null || owner != null) {
                int i = N - 1;
                while (i >= 0) {
                    T op = (UndoOperation) this.mOperations.get(i);
    ...
    public int forgetUndos(UndoOwner[] owners, int count) {
        ...
        for (int i = MERGE_MODE_NONE; i < this.mUndos.size() && removed < count; i += MERGE_MODE_UNIQUE) {
    ...
    public int forgetRedos(UndoOwner[] owners, int count) {
        ...
        int removed = MERGE_MODE_NONE;
        for (int i = MERGE_MODE_NONE; i < this.mRedos.size() && removed < count; i += MERGE_MODE_UNIQUE) {
    ...
    UndoState getTopUndo(UndoOwner[] owners) {
        ...
        if (i >= 0) {
            return (UndoState) this.mUndos.get(i);
        }
        return null;
    }
    ...
    UndoState getTopRedo(UndoOwner[] owners) {
        ...
        if (i >= 0) {
            return (UndoState) this.mRedos.get(i);
        }
        return null;
    }
