...
public class StackView extends AdapterViewAnimator {
    ...
    private static class HolographicHelper {
        ...
        HolographicHelper(Context context) {
            ...
            this.mSmallBlurMaskFilter = new BlurMaskFilter(2.0f * this.mDensity, Blur.NORMAL);
            this.mLargeBlurMaskFilter = new BlurMaskFilter(4.0f * this.mDensity, Blur.NORMAL);
        }
    ...
    private class StackSlider {
        ...
        private float highlightAlphaInterpolator(float r) {
            if (r < Face.CONFIDENCE_THRESHOLD) {
                return cubic(r / Face.CONFIDENCE_THRESHOLD) * 0.85f;
            }
            return cubic(android.view.WindowManager.LayoutParams.BRIGHTNESS_OVERRIDE_FULL - ((r - Face.CONFIDENCE_THRESHOLD) / (android.view.WindowManager.LayoutParams.BRIGHTNESS_OVERRIDE_FULL - Face.CONFIDENCE_THRESHOLD))) * 0.85f;
        }

        private float viewAlphaInterpolator(float r) {
            if (r > 0.3f) {
                return (r - 0.3f) / (android.view.WindowManager.LayoutParams.BRIGHTNESS_OVERRIDE_FULL - 0.3f);
            }
        ...
        private float rotationInterpolator(float r) {
            ...
            return (r - StackView.SWIPE_THRESHOLD_RATIO) / (android.view.WindowManager.LayoutParams.BRIGHTNESS_OVERRIDE_FULL - StackView.SWIPE_THRESHOLD_RATIO);
        }
        ...
        private float getDuration(boolean invert, float velocity) {
            ...
            float maxd = (float) Math.sqrt(Math.pow((double) StackView.this.mSlideAmount, 2.0d) + Math.pow((double) (Face.CONFIDENCE_THRESHOLD * ((float) StackView.this.mSlideAmount)), 2.0d));
            if (velocity == StackView.PERSPECTIVE_SCALE_FACTOR) {
                float f;
                if (invert) {
                    f = android.view.WindowManager.LayoutParams.BRIGHTNESS_OVERRIDE_FULL - (d / maxd);
                } else {
                    f = d / maxd;
                }
                return f * SensorManager.LIGHT_SUNRISE;
            }
            float duration = invert ? d / Math.abs(velocity) : (maxd - d) / Math.abs(velocity);
            if (duration < 50.0f || duration > SensorManager.LIGHT_SUNRISE) {
    ...
    void transformViewForTransition(int fromIndex, int toIndex, View view, boolean animate) {
        ...
        if (fromIndex == INVALID_POINTER && toIndex == getNumActiveViews() + INVALID_POINTER) {
            ...
            if (animate) {
                postDelayed(new C10561(view), MIN_TIME_BETWEEN_SCROLLS);
            } else {
                view.setAlpha(PERSPECTIVE_SCALE_FACTOR);
            }
        }
    ...
    private void transformViewAtIndex(int index, View view, boolean animate) {
        ...
        float scale = android.view.WindowManager.LayoutParams.BRIGHTNESS_OVERRIDE_FULL - (PERSPECTIVE_SCALE_FACTOR * (android.view.WindowManager.LayoutParams.BRIGHTNESS_OVERRIDE_FULL - r));
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        for (int i = ITEMS_SLIDE_UP; i < childCount; i += ITEMS_SLIDE_DOWN) {
            ...
            if ((lp.horizontalOffset == 0 && lp.verticalOffset == 0) || child.getAlpha() == PERSPECTIVE_SCALE_FACTOR || child.getVisibility() != 0) {
    ...
    private void onLayout() {
        ...
        int newSlideAmount = Math.round(SLIDE_UP_RATIO * ((float) getMeasuredHeight()));
        if (this.mSlideAmount != newSlideAmount) {
            this.mSlideAmount = newSlideAmount;
            this.mSwipeThreshold = Math.round(SWIPE_THRESHOLD_RATIO * ((float) newSlideAmount));
        }
        ...
        if (((int) Math.abs(deltaY)) > this.mTouchSlop && this.mSwipeGestureType == 0) {
            ...
            if (this.mAdapter != null) {
                ...
                if (this.mStackMode != 0) {
                    activeIndex = swipeGestureType == GESTURE_SLIDE_DOWN ? ITEMS_SLIDE_DOWN : ITEMS_SLIDE_UP;
                } else if (swipeGestureType == GESTURE_SLIDE_DOWN) {
                    activeIndex = ITEMS_SLIDE_UP;
                } else {
                    activeIndex = ITEMS_SLIDE_DOWN;
                }
                boolean endOfStack;
                if (this.mLoopViews && adapterCount == ITEMS_SLIDE_DOWN && ((this.mStackMode == 0 && swipeGestureType == ITEMS_SLIDE_DOWN) || (this.mStackMode == ITEMS_SLIDE_DOWN && swipeGestureType == GESTURE_SLIDE_DOWN))) {
                    endOfStack = true;
                } else {
                    endOfStack = false;
                }
                boolean beginningOfStack;
                if (this.mLoopViews && adapterCount == ITEMS_SLIDE_DOWN && ((this.mStackMode == ITEMS_SLIDE_DOWN && swipeGestureType == ITEMS_SLIDE_DOWN) || (this.mStackMode == 0 && swipeGestureType == GESTURE_SLIDE_DOWN))) {
                    beginningOfStack = true;
                } else {
    ...
    private void handlePointerUp(MotionEvent ev) {
        ...
        if (this.mVelocityTracker != null) {
            this.mVelocityTracker.computeCurrentVelocity(android.view.WindowManager.LayoutParams.TYPE_APPLICATION_PANEL, (float) this.mMaximumVelocity);
        ...
        if (deltaY > this.mSwipeThreshold && this.mSwipeGestureType == GESTURE_SLIDE_DOWN && this.mStackSlider.mMode == 0) {
            ...
            finalYProgress = this.mStackMode == ITEMS_SLIDE_DOWN ? android.view.WindowManager.LayoutParams.BRIGHTNESS_OVERRIDE_FULL : PERSPECTIVE_SCALE_FACTOR;
            ...
            finalYProgress = this.mStackMode == ITEMS_SLIDE_DOWN ? PERSPECTIVE_SCALE_FACTOR : android.view.WindowManager.LayoutParams.BRIGHTNESS_OVERRIDE_FULL;
    ...
    public void advance() {
        ...
        if (this.mAdapter != null) {
            if (!(getCount() == ITEMS_SLIDE_DOWN && this.mLoopViews) && this.mSwipeGestureType == 0 && timeSinceLastInteraction > 5000) {
                showNext();
            }
        }
    }
    ...
    private void measureChildren() {
        ...
        this.mNewPerspectiveShiftX = PERSPECTIVE_SHIFT_FACTOR_Y * ((float) measuredWidth);
        this.mNewPerspectiveShiftY = PERSPECTIVE_SHIFT_FACTOR_Y * ((float) measuredHeight);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (heightSpecMode == 0) {
            if (haveChildRefSize) {
                heightSpecSize = (Math.round(((float) this.mReferenceChildHeight) * (android.view.WindowManager.LayoutParams.BRIGHTNESS_OVERRIDE_FULL + 1.1111112f)) + this.mPaddingTop) + this.mPaddingBottom;
            } else {
                heightSpecSize = ITEMS_SLIDE_UP;
            }
        } else if (heightSpecMode == RtlSpacingHelper.UNDEFINED) {
            if (haveChildRefSize) {
                int height = (Math.round(((float) this.mReferenceChildHeight) * (android.view.WindowManager.LayoutParams.BRIGHTNESS_OVERRIDE_FULL + 1.1111112f)) + this.mPaddingTop) + this.mPaddingBottom;
                if (height <= heightSpecSize) {
                    heightSpecSize = height;
                } else {
                    heightSpecSize |= WindowManagerPolicy.FLAG_INJECTED;
                }
            } else {
                heightSpecSize = ITEMS_SLIDE_UP;
            }
        ...
        if (widthSpecMode == 0) {
            if (haveChildRefSize) {
                widthSpecSize = (Math.round(((float) this.mReferenceChildWidth) * (android.view.WindowManager.LayoutParams.BRIGHTNESS_OVERRIDE_FULL + 1.1111112f)) + this.mPaddingLeft) + this.mPaddingRight;
            } else {
            ...
            if (haveChildRefSize) {
                ...
                if (width <= widthSpecSize) {
                    ...
                    widthSpecSize |= WindowManagerPolicy.FLAG_INJECTED;
                }
            } else {
                widthSpecSize = ITEMS_SLIDE_UP;
            }
