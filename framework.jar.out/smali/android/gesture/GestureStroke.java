    ...
    public GestureStroke(ArrayList<GesturePoint> points) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (bx == null) {
                ...
                len = (float) (((double) len) + Math.sqrt(Math.pow((double) (p.f7x - tmpPoints[(i - 1) * 2]), 2.0d) + Math.pow((double) (p.f8y - tmpPoints[((i - 1) * 2) + 1]), 2.0d)));
                bx.union(p.f7x, p.f8y);
            }
            index++;
        }
