...
public abstract class WallpaperService extends Service {
    ...
    public class Engine {
        ...
        class C07021 extends BaseSurfaceHolder {
            C07021() {
        ...
        class C07032 extends BaseIWindow {
            C07032() {
            }
            ...
            public void resized(Rect frame, Rect overscanInsets, Rect contentInsets, Rect visibleInsets, Rect stableInsets, boolean reportDraw, Configuration newConfig) {
                Engine.this.mCaller.sendMessage(Engine.this.mCaller.obtainMessageI(WallpaperService.MSG_WINDOW_RESIZED, reportDraw ? 1 : 0));
            }
        ...
        class C07043 implements DisplayListener {
            C07043() {
            }
        ...
        public Engine() {
            ...
            this.mSurfaceHolder = new C07021();
            this.mWindow = new C07032();
            this.mDisplayListener = new C07043();
        }
        ...
        void updateSurface(boolean forceRelayout, boolean forceReport, boolean redrawNeeded) {
            ...
            boolean creating = !this.mCreated ? true : WallpaperService.DEBUG;
            boolean surfaceCreating = !this.mSurfaceCreated ? true : WallpaperService.DEBUG;
            ...
            boolean insetsChanged = !this.mCreated ? true : WallpaperService.DEBUG;
            boolean typeChanged = this.mType != this.mSurfaceHolder.getRequestedType() ? true : WallpaperService.DEBUG;
            boolean flagsChanged = (this.mCurWindowFlags == this.mWindowFlags && this.mCurWindowPrivateFlags == this.mWindowPrivateFlags) ? WallpaperService.DEBUG : true;
            if (forceRelayout || creating || surfaceCreating || formatChanged || z || typeChanged || flagsChanged || redrawNeeded || !this.mIWallpaperEngine.mShownReported) {
                try {
                    ...
                    this.mCurWindowFlags = this.mWindowFlags;
                    this.mLayout.flags = ((this.mWindowFlags | AccessibilityNodeInfo.ACTION_PREVIOUS_AT_MOVEMENT_GRANULARITY) | InputMethodManager.CONTROL_START_INITIAL) | 8;
                    this.mCurWindowPrivateFlags = this.mWindowPrivateFlags;
                    this.mLayout.privateFlags = this.mWindowPrivateFlags;
                    this.mLayout.memoryType = this.mType;
                    this.mLayout.token = this.mWindowToken;
                    if (!this.mCreated) {
                        WindowManager windowService = (WindowManager) WallpaperService.this.getSystemService(Context.WINDOW_SERVICE);
                        TypedArray windowStyle = WallpaperService.this.obtainStyledAttributes(R.styleable.Window);
                        if ((windowService.getDefaultDisplay().getDisplayId() == 0 ? true : WallpaperService.DEBUG) && windowStyle.hasValue(52)) {
                            if (this.mOutsetBottom == null) {
                                this.mOutsetBottom = new TypedValue();
                            }
                            windowStyle.getValue(52, this.mOutsetBottom);
                        } else {
                            this.mOutsetBottom = null;
                        }
                        this.mWindowIsRound = WallpaperService.this.getResources().getBoolean(17956989);
                        windowStyle.recycle();
                        this.mIsEmulator = Build.HARDWARE.contains("goldfish");
                        this.mIsCircularEmulator = SystemProperties.getBoolean(ViewRootImpl.PROPERTY_EMULATOR_CIRCULAR, WallpaperService.DEBUG);
                        ...
                        this.mLayout.windowAnimations = 16974565;
                        this.mInputChannel = new InputChannel();
                        if (this.mSession.addToDisplay(this.mWindow, this.mWindow.mSeq, this.mLayout, 0, 0, this.mContentInsets, this.mStableInsets, this.mInputChannel) < 0) {
                    ...
                    int relayoutResult = this.mSession.relayout(this.mWindow, this.mWindow.mSeq, this.mLayout, this.mWidth, this.mHeight, 0, 0, this.mWinFrame, this.mOverscanInsets, this.mContentInsets, this.mVisibleInsets, this.mStableInsets, this.mConfiguration, this.mSurfaceHolder.mSurface);
                    ...
                    if (!fixedSize) {
                        ...
                        Rect rect = this.mOverscanInsets;
                    ...
                    insetsChanged = ((insetsChanged | (!this.mDispatchedOverscanInsets.equals(this.mOverscanInsets) ? 1 : 0)) | (!this.mDispatchedContentInsets.equals(this.mContentInsets) ? 1 : 0)) | (!this.mDispatchedStableInsets.equals(this.mStableInsets) ? 1 : 0);
                    ...
                    if (this.mSurfaceHolder.mSurface.isValid()) {
                        ...
                        if (insetsChanged) {
                            ...
                            boolean isRound = ((this.mIsEmulator && this.mIsCircularEmulator) || this.mWindowIsRound) ? true : WallpaperService.DEBUG;
                            this.mFinalSystemInsets.set(this.mDispatchedOverscanInsets);
                            this.mFinalStableInsets.set(this.mDispatchedStableInsets);
                            if (this.mOutsetBottom != null) {
                                this.mFinalSystemInsets.bottom = ((int) this.mOutsetBottom.getDimension(WallpaperService.this.getResources().getDisplayMetrics())) + this.mIWallpaperEngine.mDisplayPadding.bottom;
                            }
                            onApplyWindowInsets(new WindowInsets(this.mFinalSystemInsets, null, this.mFinalStableInsets, isRound));
                        }
                        if (redrawNeeded) {
        ...
        void reportVisibility() {
            ...
            if (!this.mDestroyed) {
                if (((this.mDisplay != null ? 1 : 0) & this.mVisible) == 0 || this.mDisplay.getState() == 1) {
                    visible = WallpaperService.DEBUG;
                }
                ...
                if (always || this.mOffsetsChanged) {
                    ...
                    if (this.mSurfaceCreated) {
                        if (this.mReportedVisible) {
                            ...
                            if (availw > 0) {
                                xPixels = -((int) ((((float) availw) * xOffset) + 0.5f));
                            } else {
                                xPixels = 0;
                            }
        ...
        void doCommand(WallpaperCommand cmd) {
            ...
            if (this.mDestroyed) {
                ...
                result = onCommand(cmd.action, cmd.f82x, cmd.f83y, cmd.f84z, cmd.extras, cmd.sync);
            }
            if (cmd.sync) {
                try {
    ...
    class IWallpaperEngineWrapper extends Stub implements HandlerCaller.Callback {
        ...
        IWallpaperEngineWrapper(WallpaperService context, IWallpaperConnection conn, IBinder windowToken, int windowType, boolean isPreview, int reqWidth, int reqHeight, Rect padding) {
            ...
            this.mCaller.sendMessage(this.mCaller.obtainMessage(WallpaperService.DO_ATTACH));
        }
            ...
            switch (message.what) {
                ...
                case WallpaperService.MSG_WINDOW_RESIZED /*10030*/:
                    ...
                    if (message.arg1 != 0) {
                        ...
                        reportDraw = WallpaperService.DEBUG;
                    }
