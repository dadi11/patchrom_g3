...
public class AlwaysOnHotwordDetector {
    ...
    class RefreshAvailabiltyTask extends AsyncTask<Void, Void, Void> {
        ...
        public Void doInBackground(Void... params) {
            ...
            if (availability == 0 || availability == AlwaysOnHotwordDetector.STATE_KEYPHRASE_UNENROLLED || availability == AlwaysOnHotwordDetector.STATE_KEYPHRASE_ENROLLED) {
    ...
    public boolean startRecognition(int recognitionFlags) {
        ...
        synchronized (this.mLock) {
            if (this.mAvailability == STATE_INVALID) {
                ...
                z = startRecognitionLocked(recognitionFlags) == 0 ? true : DBG;
            }
        }
        return z;
    }
    ...
    public boolean stopRecognition() {
        ...
        synchronized (this.mLock) {
            if (this.mAvailability == STATE_INVALID) {
                ...
                z = stopRecognitionLocked() == 0 ? true : DBG;
            }
        }
        return z;
    }
    ...
    void onSoundModelsChanged() {
        synchronized (this.mLock) {
            if (this.mAvailability == STATE_INVALID || this.mAvailability == STATE_HARDWARE_UNAVAILABLE || this.mAvailability == STATE_KEYPHRASE_UNSUPPORTED) {
    ...
    private int startRecognitionLocked(int recognitionFlags) {
        ...
        if ((recognitionFlags & STATE_KEYPHRASE_UNENROLLED) != 0) {
            captureTriggerAudio = true;
        } else {
            captureTriggerAudio = DBG;
        }
        if ((recognitionFlags & STATE_KEYPHRASE_ENROLLED) == 0) {
            allowMultipleTriggers = DBG;
        }
