...
public abstract class BatteryStats implements Parcelable {
    ...
    class C05801 implements Comparator<TimerEntry> {
        C05801() {
        }
            ...
            if (rec.cmd == BatteryStats.WIFI_SUPPL_STATE_SCANNING) {
                ...
                if (!checkin) {
                    ...
                    if (verbose) {
                        pw.print(" ");
                        if (rec.states >= 0) {
                            if (rec.states < BatteryStats.DUMP_VERBOSE) {
                                pw.print("0000000");
                            } else if (rec.states < InputMethodManager.CONTROL_START_INITIAL) {
                                pw.print("000000");
                            } else if (rec.states < AccessibilityNodeInfo.ACTION_SCROLL_FORWARD) {
                                pw.print("00000");
                            } else if (rec.states < AccessibilityNodeInfo.ACTION_CUT) {
                                pw.print("0000");
                            } else if (rec.states < AccessibilityNodeInfo.ACTION_DISMISS) {
                                pw.print("000");
                            } else if (rec.states < WindowManagerPolicy.FLAG_INJECTED) {
                                pw.print("00");
                            } else if (rec.states < EditorInfo.IME_FLAG_NO_EXTRACT_UI) {
                                pw.print(WifiEnterpriseConfig.ENGINE_DISABLE);
                            }
                        }
                ...
                if (this.oldPlug != rec.batteryPlugType) {
                    ...
                    switch (this.oldPlug) {
                        case BatteryStats.WIFI_SUPPL_STATE_INVALID /*0*/:
                            pw.print(checkin ? "n" : Parameters.EFFECT_NONE);
                            break;
                        case BatteryStats.WIFI_SUPPL_STATE_DISCONNECTED /*1*/:
                ...
                BatteryStats.printBitDescriptions(pw, this.oldState, rec.states, rec.wakelockTag, BatteryStats.HISTORY_STATE_DESCRIPTIONS, !checkin ? true : BatteryStats.LOCAL_LOGV);
                BatteryStats.printBitDescriptions(pw, this.oldState2, rec.states2, null, BatteryStats.HISTORY_STATE2_DESCRIPTIONS, !checkin ? true : BatteryStats.LOCAL_LOGV);
                ...
                if (rec.eventCode != 0) {
                    pw.print(checkin ? "," : " ");
                    if ((rec.eventCode & AccessibilityNodeInfo.ACTION_PASTE) != 0) {
                        pw.print("+");
                    } else if ((rec.eventCode & AccessibilityNodeInfo.ACTION_COPY) != 0) {
                        pw.print("-");
                    }
...
*/
    ...
    static {
        ...
        strArr[WIFI_SUPPL_STATE_DISCONNECTED] = "c";
        ...
        strArr2[WIFI_SUPPL_STATE_INACTIVE] = Parameters.ZSL_OFF;
        ...
        strArr3[WIFI_SUPPL_STATE_INACTIVE] = Parameters.ZSL_OFF;
        ...
        r7[WIFI_SUPPL_STATE_INVALID] = new BitDescription(RtlSpacingHelper.UNDEFINED, "low_power", "lp");
        ...
        r7[WIFI_SUPPL_STATE_INACTIVE] = new BitDescription(EditorInfo.IME_FLAG_NO_EXTRACT_UI, Context.WIFI_SERVICE, "W");
        ...
        strArr[WIFI_SUPPL_STATE_INVALID] = Parameters.ZSL_OFF;
    ...
    public final void dumpCheckinLocked(Context context, PrintWriter pw, int which, int reqUid, boolean wifiOnly) {
        ...
        if (sippers != null && sippers.size() > 0) {
            ...
            for (i = WIFI_SUPPL_STATE_INVALID; i < sippers.size(); i += WIFI_SUPPL_STATE_DISCONNECTED) {
                ...
                switch (C05812.$SwitchMap$com$android$internal$os$BatterySipper$DrainType[bs.drainType.ordinal()]) {
    ...
    public final void dumpLocked(Context context, PrintWriter pw, String prefix, int which, int reqUid, boolean wifiOnly) {
        ...
        pw.println(DateFormat.format((CharSequence) "yyyy-MM-dd-HH-mm-ss", getStartClockTime()).toString());
        long screenOnTime = getScreenOnTime(rawRealtime, which);
        long interactiveTime = getInteractiveTime(rawRealtime, which);
        long lowPowerModeEnabledTime = getLowPowerModeEnabledTime(rawRealtime, which);
        long phoneOnTime = getPhoneOnTime(rawRealtime, which);
        ...
        Comparator<TimerEntry> c05801 = new C05801();
        if (reqUid < 0) {
            TimerEntry timer2;
            Map<String, ? extends Timer> kernelWakelocks = getKernelWakelockStats();
            if (kernelWakelocks.size() > 0) {
                ...
                if (ktimers.size() > 0) {
                    Collections.sort(ktimers, c05801);
                    pw.print(prefix);
            ...
            if (timers.size() > 0) {
                Collections.sort(timers, c05801);
                pw.print(prefix);
            ...
            if (wakeupReasons.size() > 0) {
                ...
                Collections.sort(reasons, c05801);
