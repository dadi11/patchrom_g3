...
public abstract class WindowOrientationListener {
    ...
    final class SensorEventListenerImpl implements SensorEventListener {
        ...
        private static final long PROPOSAL_MIN_TIME_SINCE_ACCELERATION_ENDED_NANOS = 250000000;
        private static final long PROPOSAL_MIN_TIME_SINCE_FLAT_ENDED_NANOS = 250000000;
        private static final long PROPOSAL_MIN_TIME_SINCE_SWING_ENDED_NANOS = 150000000;
        private static final long PROPOSAL_SETTLE_TIME_NANOS = 20000000;
        ...
        private boolean isPredictedRotationAcceptableLocked(long now) {
            if (now >= this.mPredictedRotationTimestampNanos + PROPOSAL_SETTLE_TIME_NANOS && now >= this.mFlatTimestampNanos + PROPOSAL_MIN_TIME_SINCE_FLAT_ENDED_NANOS && now >= this.mSwingTimestampNanos + PROPOSAL_MIN_TIME_SINCE_SWING_ENDED_NANOS && now >= this.mAccelerationTimestampNanos + PROPOSAL_MIN_TIME_SINCE_FLAT_ENDED_NANOS) {
                return true;
            }
            return WindowOrientationListener.LOG;
        }
    ...
    private WindowOrientationListener(Context context, Handler handler, int rate) {
        ...
        this.mRate = rate;
        this.mSensor = null;
        if (context.getResources().getBoolean(17957015)) {
            this.mSensor = this.mSensorManager.getDefaultSensor(33171011);
        }
